...
data = read.csv('cluster-nettohoffnung.csv')
...
high <- high %>%
  gather(key = "time", value = "Nettohoffnung", Nettohoffnung_1, Nettohoffnung_3) %>%
  convert_as_factor(id_nummer, gruppe_3)

bxp <- ggboxplot(
  high, x = "time", y = "Nettohoffnung",
...
descr = high %>%
  ...
  get_summary_stats(Nettohoffnung, type = "mean_sd")

res.aov <- anova_test(
  data = high, dv = Nettohoffnung, wid = id_nummer,
...
res.aov <- anova_test(
  data = low, dv = Nettohoffnung, wid = id_nummer,
  between = gruppe_3, within = time
)
anova = get_anova_table(res.aov)
