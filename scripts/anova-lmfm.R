...
data = read.csv('cluster-lmfm.csv')
...
high <- high %>%
  gather(key = "time", value = "lmfm", lmfm_1, lmfm_3) %>%
...
descr = high %>%
  ...
  get_summary_stats(lmfm, type = "mean_sd")

res.aov <- anova_test(
  data = high, dv = lmfm, wid = id_nummer,
...
res.aov <- anova_test(
  data = low, dv = lmfm, wid = id_nummer,
  between = gruppe_3, within = time
)
anova = get_anova_table(res.aov)
