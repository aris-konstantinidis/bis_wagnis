...
data = read.csv('cluster-genl.csv')
...
high <- high %>%
  gather(key = "time", value = "genl", genl_1, genl_3) %>%
...
descr = high %>%
  ...
  get_summary_stats(genl, type = "mean_sd")

res.aov <- anova_test(
  data = high, dv = genl, wid = id_nummer,
...
res.aov <- anova_test(
  data = low, dv = genl, wid = id_nummer,
  between = gruppe_3, within = time
)
anova = get_anova_table(res.aov)
